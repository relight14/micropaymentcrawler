...
- **Anonymous User Support**: Unauthenticated users can chat freely without persistence. Messages display in UI but skip database saves. Upon login, conversation history is automatically preserved via localStorage bridge pattern: anonymous sessions are captured before login (survives page reload), validated with 10-minute expiration, and restored post-login by creating a new project with all messages from the session, ensuring seamless continuity.
...
- **Project Message History**: ProjectManager loads and displays saved messages when switching between projects via loadProjectMessages() method. Uses dependency injection pattern with messageCoordinator and appState for proper message hydration. When project is loaded, chat UI is cleared while preserving mode, messages are fetched from API, AppState is hydrated, and messages are rendered through MessageCoordinator ensuring UI/state synchronization.
- **Report Status Management**: Consolidated report/enrichment status tracking in MessageCoordinator (single source of truth). Five-state machine: idle → pricing → generating → complete → error. Emits REPORT_STATUS_CHANGED events for subscribers. AppState provides adapter methods during migration period for backward compatibility.
...
- **Research Query Persistence**: Complete pipeline ensures research topics persist with projects. Backend schema includes `research_query` field (PostgreSQL + SQLite), auto-captured during searches. ProjectStore is the single source of truth for project metadata, syncing to AppState on project load/creation. Report generation uses intelligent fallback chain: explicit param → AppState → ProjectStore → generic fallback, preventing "Research Query" placeholder when users return to projects. Migration-safe ALTER TABLE with existence checks ensures smooth deployment.
