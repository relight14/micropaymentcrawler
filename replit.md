...
- **Anonymous User Support**: Unauthenticated users can chat freely without persistence. Messages display in UI but skip database saves. Upon login, conversation history is automatically preserved by creating a new project with all messages from the anonymous session, ensuring seamless continuity.
