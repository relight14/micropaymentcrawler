...
async def register_content(
    ...
    try:
        ...
        if os.getenv("LEDEWIRE_MOCK_PAYMENTS") == "true":
            # MOCK MODE: Return fake content_id
            content_id = f"mock_content_{uuid.uuid4().hex[:8]}"
            logger.info(f"📝 [REGISTER] Mock mode - content_id={content_id}")
        else:
            ...
            if cached:
                ...
                logger.info(f"📝 [REGISTER] Using cached content_id={content_id}")
            else:
                ...
                try:
                    ...
                    logger.info(f"📝 [REGISTER] Content registered and cached: content_id={content_id}")
